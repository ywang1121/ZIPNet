...
class convNeXTUnetBig(nn.Module):
    # def __init__(self, net, joint_num, pretrain, deconv_dim=128, out_dim_list=[3*14, 14, 14]):
    def __init__(self, net, pretrain, deconv_dim=128):
    ...
    def forward(self, img):
        ...
        return pcl_feature, c4  # 可能用于需要结合细节和语义信息的任务，如三维重建、点云处理或图像分割,而 c4 可能用于需要高层语义理解的任务，如图像分类或对象检测
