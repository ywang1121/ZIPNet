...
class standard_transformer(nn.Module):
    def __init__(self, small_dim=1024, large_dim=512, k=16, class_num=40):
        ...
        self.net = AttBlock(large_dim, small_dim, 2)
...
class PointCAT(nn.Module):
    ...
    def __init__(self, small_dim=1024, large_dim=512, k=16, class_num=40):
        ...
        self.cross_attn_layer = CrossAttBlock(large_dim=large_dim, small_dim=small_dim, cross_attn_depth=2,
                                              cross_attn_heads=8, channels=small_dim)
...
class CrossAttBlock(nn.Module):
    def __init__(self, large_dim, small_dim, cross_attn_depth, cross_attn_heads, channels):
        ...
        self.cross_att1 = CrossAttEncoder(small_dim=small_dim, large_dim=large_dim, cross_attn_depth=cross_attn_depth,
                                          cross_attn_heads=cross_attn_heads, large_channel=large_dim,small_channel=small_dim)
        ...
        # self.conv1d = nn.Conv1d(1024, 512, kernel_size=1)
    ...
    def forward(self, xl, xs):
        ...
        xs = xs[:, 1:, :]
        # xl = self.fc(xl)
        # combined = torch.cat((xl, xs), dim=-1)
...
class AttBlock(nn.Module):
    ...
    def forward(self, xl, xs):
        # batch_size, _, _ = xl.shape
        batch_size, joint_num, _ = xl.shape
        xl = self.att1(xl)
        xs = self.att2(xs)
        xl = xl[:,1:,:]
        xs = xs[:,1:,:]
...
class CrossAttEncoder(nn.Module):
    def __init__(self, small_dim=128, large_dim=256, cross_attn_depth=4, cross_attn_heads=4, large_channel=512,small_channel=256):
        super(CrossAttEncoder, self).__init__()

        self.ln_ls1 = nn.LayerNorm(small_channel)
        self.ln_ls2 = nn.LayerNorm(small_channel)
        self.ln_sl1 = nn.LayerNorm(large_channel)
        self.ln_sl2 = nn.LayerNorm(large_channel)
