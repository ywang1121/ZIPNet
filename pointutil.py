...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, mlp2=None, group_all=False, use_fps=True,
        ...
        if mlp2:
            for out_channel in mlp2:
                ...
                last_channel = out_channel
        if knn is False:
            # self.queryandgroup = pointnet2_utils.QueryAndGroup_base(radius, nsample, self.use_xyz)
    ...
    def forward(self, xyz, points, new_xyz = None, fps_idx=None):
        ...
        if new_xyz == None:
            if (self.group_all == False) and (self.npoint != -1) and self.use_fps:
                if fps_idx == None:
                    ...
                    #####################################################################
                    if N == 1024:
                        fps_idx = furthest_point_sample(xyz_t, self.npoint)  # [B, N]  add
                        fps_idx = torch.sort(fps_idx, dim=-1)[0]
                        new_xyz = index_points(xyz_t, fps_idx)
                    else:
                        fps_idx = furthest_point_sample(xyz, self.npoint)  # [B, N]  add
                        fps_idx = torch.sort(fps_idx, dim=-1)[0]
                        new_xyz = index_points(xyz_t, fps_idx)
                    ######################################################################
                # new_xyz = pointnet2_utils.gather_operation(xyz, fps_idx)  # [B, C, N]  采样后的点云坐标，用于代表整个点云的子集.(nter)
                ...
                new_xyz = xyz  # (32,3,128)

        if self.knn:
            # sqrdists = square_distance(new_xyz.transpose(2, 1).contiguous(), xyz_t)
            sqrdists = square_distance(new_xyz.contiguous(), xyz_t)
            ...
            direction_xyz = neighbor_xyz - new_xyz.view(B, self.npoint, 1, C)
            ...
            # new_points = self.queryandgroup(xyz_t, new_xyz.transpose(2, 1).contiguous(), points)  # [B, 3+C, N, S] (32.131.512.64)
            # if N == 1024:
            #     new_points = self.queryandgroup(xyz_t, new_xyz.contiguous(), points)  # [B, 3+C, N, S] (32.131.512.64)
            # else:
            #     new_points = self.queryandgroup(xyz_t, new_xyz.contiguous(), points)  # [B, 3+C, N, S] (32.131.512.64)
            # new_points = self.queryandgroup(xyz_t, new_xyz.contiguous(), points)  # [B, 3+C, N, S] (32.131.512.64)
            new_points ,adjusted_radius= self.queryandgroup(xyz_t, new_xyz.contiguous(), points)  # [B, 3+C, N, S] (32.131.512.64)
        ...
        if self.mean_aggr:
            ...
            new_points = torch.max(new_points, -1)[0]  # (32,128/256,512/128) 从每个采样点的局部邻域中提取全局特征
        ...
        if self.return_fps:
            ...
            return new_xyz.permute(0, 2, 1), new_points,adjusted_radius
...
def index_points_group_self(points, knn_idx):
    """
        Input:
            points: input points data, [B, N, C]
    ...
    # 初始化输出张量
    new_points = torch.zeros(B, S, K, C, dtype=points.dtype, device=points.device)

    # 为每个批次进行循环
    for b in range(B):
        for s in range(S):
            # 从 knn_idx 中获取当前批次和采样点的索引
            idx = knn_idx[b, s]

            # 使用索引从 points 中取出对应的点
            new_points[b, s, :, :] = points[b, idx, :]


    # knn_idx = knn_idx.view(B, S, K, 1).expand(-1, -1, -1, C)

    # new_points = torch.gather(points.unsqueeze(1), 2, knn_idx)
