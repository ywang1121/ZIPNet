...
class loader(Dataset):
    ...
    def uvd_nl2xyznl_tensor(self, uvd, center, m, cube, cam_paras):
        ...
        M_t = m.to(device).view(batch_size, 1, 3, 3)
        # M_inverse = torch.linalg.inv(M_t).repeat(1, point_num, 1, 1)
        M_inverse = torch.linalg.inv(M_t.cpu()).cuda().repeat(1, point_num, 1, 1)
...
def visualize_point_cloud(pcl_sample):
    # 将点云数据转换为 Open3D 的 PointCloud 格式
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(pcl_sample)
...
class nyu_loader(loader):
    ...
    def __getitem__(self, index):
        ...
        # save_path = 'path_to_save_depth_crop.png'
        # cv2.imwrite(save_path, depth_crop)

        if self.phase == 'train':
            mode, off, rot, sc = self.rand_augment(sigma_com=self.aug_para[0], sigma_sc=self.aug_para[1], rot_range=self.aug_para[2])
            imgD,imgD_unnorm, curLabel,  cube, com2D, M, _ = self.augmentCrop(depth_crop, gt3Dcrop, center_uvd, self.cube_size,
                                                                    trans, mode, off, rot, sc, self.paras)

            # self.RandDotPercentage = 0.4
            # img = np.expand_dims(imgD, axis=0)
            # img = torch.from_numpy(img).float()
            # if self.RandDotPercentage > 0 and np.random.rand() < 0.7:
            #     p = np.random.rand() * self.RandDotPercentage
            #     imgD= PixDropout(img, background_value=1, P=p, V=1)
            #     imgD = imgD.squeeze(0).numpy()

            self.RandDotPercentage = 0.4
            img = np.expand_dims(imgD, axis=0)
            img = torch.from_numpy(img).float()
            if self.RandDotPercentage > 0 and np.random.rand() < 0.7:
                p = self.RandDotPercentage
                # p = np.random.rand() * self.RandDotPercentage
                imgD = PixDropout(img, background_value=1, P=p, V=1)
                imgD = imgD.squeeze(0).numpy()


            curLabel_unnorm = curLabel.copy()
        ...
        joint_img_unnorm = joint_img.copy()
        ...
        # get pcl 提取并下采样点云
        # if self.phase == 'train':
        #     pcl = self.getpcl(imgD_ori, com3D, cube, M)
        # else:
        ...
        if pcl_num == 0:
            ...
            pcl_sample_numpy = pcl_sample.copy()

        # fx = self.paras[0]
        # fy = self.paras[1]
        # u0 = self.paras[2]
        # v0 = self.paras[3]
        # visualize_point_cloud(pcl_sample)
        data = torch.from_numpy(imgD).float()
        data = data.unsqueeze(0)
        # img_norm = torch.from_numpy(imgD_norm).float()   #add
        # img_norm = img_norm.unsqueeze(0)
        pcl_sample = torch.from_numpy(pcl_sample).float()
        joint_img = torch.from_numpy(joint_img).float()
        # joint_uvd = torch.from_numpy(joint_img_unnorm).float()  # add
        joint = torch.from_numpy(curLabel).float()
        # joint_xyz = torch.from_numpy(curLabel_unnorm).float()  #add
        # center_2d = torch.from_numpy(com2D).float()#add
        ...
        # return data,img_norm, pcl_sample, joint,joint_xyz, joint_img, joint_uvd,center, center_2d,M, cube, cam_para, cube_size[0],fx,fy,u0,v0
...
class DexYCBDataset(loader):
  ...
  def load_data(self):
      ...
      for aid in db.anns.keys():
          ...
          img_path = img['color_file_name'] #add
  ...
  def __getitem__(self, idx):
    ...
    if self.phase == 'train':
        ...
        #     imgD = imgD.squeeze(0).numpy()
        ############################################################################
        # save_path = f'./testimage/depth_aug.{idx}.png'
...
def PixholeDropout(img, background_value, max_hole_size, num_holes, V=1):
    """
    img: 输入图像，形状为 (1, H, W)
    background_value: 背景值
    max_hole_size: 洞孔的最大尺寸
    num_holes: 要生成的洞孔数量
    V: 洞孔的值
    """
    img = img.clone()
    mask = abs(img[0] - background_value) > 1e-6  # 定位前景像素
    y, x = torch.where(mask)  # 获取它们的像素坐标

    if len(x) == 0 or len(y) == 0:
        return img  # 如果没有前景像素，直接返回原图

    # 确保洞孔数量不超过前景像素的数量
    num_holes = min(num_holes, len(x))

    # 创建洞孔
    for _ in range(num_holes):
        # 随机生成洞孔的中心点坐标
        center_x = np.random.randint(0, img.shape[2])
        center_y = np.random.randint(0, img.shape[1])

        # 随机生成洞孔的大小
        hole_size = np.random.randint(1, max_hole_size + 1)

        # 确保洞孔大小不超过图像尺寸
        hole_size = min(hole_size, img.shape[1], img.shape[2])

        # 创建一个网格坐标
        grid_y, grid_x = torch.meshgrid(torch.arange(img.shape[1]), torch.arange(img.shape[2]))

        # 计算当前像素到中心点的距离
        distance = torch.sqrt((grid_y - center_y) ** 2 + (grid_x - center_x) ** 2)

        # 更新图像中的洞孔
        img[0, distance <= hole_size] = V

    return img


def histogram_matching(src, target):
    # 确保源图像和目标图像都是单通道的灰度图像
    if len(src.shape) > 2:
    ...
    target_hist, bins = np.histogram(target.flatten(), 256, [0, 256])
    ...
    for src_value in range(256):
        while target_value < 256 and target_cdf_normalized[target_value] < src_cdf_normalized[src_value]:
            target_value += 1
        lookup_table[src_value] = target_value

    # 应用查找表进行直方图匹配
    result = cv2.LUT(src, lookup_table)

    # 填补洞孔
    # result = cv2.inpaint(result, np.zeros_like(result), 3, cv2.INPAINT_TELEA)
