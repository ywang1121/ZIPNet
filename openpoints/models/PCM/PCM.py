from pointnet2.pointnet2_utils import DynamicBallRadiusAdjustment
...
from .PCM_utils import MLP, serialization, _init_weights, index_points, square_distance
...
class PointMambaEncoder(nn.Module):
    def __init__(self, in_channels=3, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 mamba_blocks=[1, 1, 1, 1],radius = 0.3,
                 ...
                 cls_pooling="max",lambda_param=0.1,k_dense=32,
                 **kwargs):
        ...
        norm_cls = partial(
            ...
            # nn.Identity if not rms_norm else RMSNorm, eps=1e-5,
        )
        ...
        for i in range(len(pre_blocks)):
            ...
            local_grouper = LocalGrouper(last_channel, reduce, kneighbor, use_xyz, normalize,
                                         k_stride=kstride,radius=radius_1,lambda_param=lambda_param,k_dense=k_dense)  # [b,g,k,d]
            self.local_grouper_list.append(local_grouper)
            self.dynamic_ball_radius_adjustment = DynamicBallRadiusAdjustment(radius_1,lambda_param,k_dense)
    ...
    def forward_cls_feat(self, p, x=None):
        ...
        for i in range(self.stages):
            ...
            # GAM forward
            p, x, x_res,adjust_radius = self.local_grouper_list[i](p, x.permute(0, 2, 1), x_res)  # [b,g,3]  [b,g,k,d]
            x = self.pre_blocks_list[i](x)  # [b,d,g] 从局部邻域中提取特征
        ...
        # pc1 = polar_to_cartesian(pc1)
        # pc2 = polar_to_cartesian(pc2)
        # return x
        return pc1,feat1,pc2,feat2,adjust_radius
        # return pc,feat
...
def cartesian_to_polar(p):
    ...
    参数:
    p -- 形状为(B, 3, N)的张量，其中B是批次大小，N是点的数量

    返回:
    r, theta, phi -- 极坐标的张量
    """
    x, y, z = p[:, 0, :], p[:, 1, :], p[:, 2, :]
    r = torch.sqrt(x ** 2 + y ** 2 + z ** 2)  # 点到原点的距离
    theta = torch.atan2(y, x)  # xy平面上的角度
    phi = torch.acos(z / r)  # 从z轴到点的线与xy平面的夹角
...
def polar_to_cartesian(p):
    ...
    参数:
    p -- 形状为(B, 3, N)的张量，其中第一维是r, θ, φ

    返回:
    p_cartesian -- 笛卡尔坐标的张量，形状为(B, 3, N)
    """
