...
#
# # # 定义关节对
# # jt_idx = [[0,3], [6,9], [12,15], [18,21], [24,25,27,30,31,32]]
# # sketch = [[(0, 3), (3, 32)],
# #           [(6, 9), (9, 32)],
# #           [(12, 15), (15, 32)],
# #           [(18, 21), (21, 32)],
# #           [(24, 25), (25, 27),(27, 32), (30, 32), (31, 32)]]
#
# def spatial_depth_ordering_loss(joints_pred, joints_gt, sketch, margin=1.0):
#     """
#     joints_pred: 预测的关节点坐标 (B, 3, N) -> (32, 3, 14)
#     joints_gt: 真实的关节点坐标 (B, 3, N) -> (32, 3, 14)
#     sketch: 关节对的索引，表示需要比较深度的关节点对
#     margin: 损失函数中的边界值
#     """
#     # 提取关节点的z坐标（深度）
#     z_pred = joints_pred[:, 2, :]  # (B, 14)
#     z_gt = joints_gt[:, 2, :]  # (B, 14)
#
#     # 初始化损失
#     loss = 0
#     num_pairs = 0
#
#     # 计算每对关节的深度差异和损失
#     for pair_list in sketch:
#         for i, j in pair_list:
#             diff_pred = z_pred[:, i] - z_pred[:, j]
#             diff_gt = z_gt[:, i] - z_gt[:, j]
#             loss += torch.clamp(margin - torch.abs(diff_pred - diff_gt), min=0) ** 2
#             num_pairs += 1
#
#     # 求平均损失，并确保返回一个标量值
#     if num_pairs > 0:
#         loss = loss.sum() / num_pairs  # 确保loss是一个标量
#     else:
#         loss = loss.sum()  # 如果没有有效的pairs，直接求和
#
#     return loss
#############################################################################################
...
# class KRegularizedLoss(nn.Module):
#     def __init__(self, k_optimal, lambda_k):
#         super(KRegularizedLoss, self).__init__()
...
#
#         # 正则化项
#         k_penalty = (k - self.k_optimal) ** 2
#
#         # 组合损失
#         loss = self.lambda_k * k_penalty
#         return loss
...
#         '''
#         print("------1----------------------------------------")
#         print('xyz1.shape:{0}'.format(xyz1.shape))
#         print('xyz2.shape:{0}'.format(xyz2.shape))
#         print('points1.shape:{0}'.format(points1.shape))
#         print('points2.shape:{0}'.format(points2.shape))
#
#
#         B, C, N1 = xyz1.shape
#         _, _, N2 = xyz2.shape
...
class HandModel(nn.Module):
    def __init__(self, joints=21, stacks=10):
        ...
        # self.backbone = MLLA(img_size=128, patch_size=2, in_chans=1, num_classes=14, embed_dim=64, depths=[2, 2, 6, 2],
        #                      dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], drop_path_rate=0.1,deconv_dim=128)
        ...
        self.fold1 = nn.Sequential(BiasConv1d(bias_length=joints, in_channels=1024, out_channels=512, bn=True),
    ...
    def encode(self, pc, feat, img, loader, center, M, cube, cam_para):
        ...
        # pc1, feat1,adjust_radius1 = self.encoder_1(pc, feat)  # B, 3, 512; B, 64, 512
        # pc2, feat2,adjust_radius2 = self.encoder_2(pc1, feat1)  # B, 3, 256; B, 128, 256

        code = self.encoder_3(torch.cat((pc2, feat2), 1))  # B, 3, 128; B, 256, 8 Super points&features F
        ...
        pcl_feat = torch.gather(pc_img_feat.view(B, C, -1), -1, pcl_feat_index).view(B, C, N, -1)  # 图像特征中提取对应点云点的特征。
        pcl_feat = torch.sum(pcl_feat * pcl_closeness.unsqueeze(1), dim=-1)  # 点云周围的 四个局部点-局部特征

        feat1 = torch.cat((feat1, pcl_feat), 1)  # 将提取的特征与之前的特征合并。
    ...
    def get_loss(self, pc, feat, img, loader, center, M, cube, cam_para, gt):
        ...
        # radius_loss = radius_regularization_loss(dynamic_radius1,radius_min=0.1,radius_max=0.3) +radius_regularization_loss(dynamic_radius2,radius_min=0.1,radius_max=0.3)
...
class HandModel_DASRT(nn.Module):
    def __init__(self, joints=21, stacks=10):
        super(HandModel_DASRT, self).__init__()
    ...
    def encode(self, pc, feat, img, loader, center, M, cube, cam_para):
        ...
        # return latents,joints, pc1, feat1,adjust_radius
        return latents,joints, pc1, feat1,adjust_radius1,adjust_radius2


    def forward(self, pc, feat, img, loader, center, M, cube, cam_para):
        # embed,joint, pc1, feat1,dynamic_radius= self.encode(pc, feat, img, loader, center, M, cube, cam_para)
        embed,joint, pc1, feat1,dynamic_radius1,dynamic_radius2= self.encode(pc, feat, img, loader, center, M, cube, cam_para)
        # joint = torch.randn_like(joint) * 0.1 + joint  # (B, d, J) original training
        # joint = torch.randn_like(joint) * 3 + joint  # (B, d, J)
...
class HandModel_base(nn.Module):
    def __init__(self, joints=21, stacks=10, iters=50):
        super(HandModel_base, self).__init__()
...
#
        ...
        #     [CrossAttention_GCN(nsample=64, in_channel=256, latent_channel=512, graph_width=joints,
        #                         mlp=[512, 512, 512], mlp2=None) for _ in range(stacks)])

        self.regress = nn.Conv1d(in_channels=512, out_channels=3, kernel_size=1)

        # self.noise = LearnableNoiseLayer()
    ...
    def encode(self, pc, feat, img, loader, center, M, cube, cam_para):
        ...
        # pc1 ,feat1 = self.encoder_1(pc)
        pc1, feat1 = self.encoder_1(pc, feat)# B, 3, 512; B, 64, 512
        pc2, feat2 = self.encoder_2(pc1, feat1)  # B, 3, 256; B, 128, 256

        # pc1, feat1, pc2, feat2,adjust_radius= self.encoder_global(pc, feat)
        #########################################################################
        # feat1, feat2 = self.cross_encoder(feat1.permute(0, 2, 1), feat2.permute(0, 2, 1))
        # feat1, feat2 = feat1.transpose(1, 2), feat2.transpose(1, 2)
        ##########################################################################
        code = self.encoder_3(torch.cat((pc2, feat2), 1))  # B, 3, 128; B, 256, 8 Super points&features F
        # code = self.encoder_3(torch.cat((pc2, feat2), 1))  # (32,512,1) Super points&features F
        #########################################################################
        # code = self.cross_localfeat(code,feat1)
        #########################################################################
        # pc_img_feat,img_code = self.backbone(
        #     img)  # img_offset: B×C×W×H , C=3(direct vector)+1(heatmap)+1(weight)

        # img_code = self.vimbackbone(img)

        pc_img_feat,c4 = self.backbone(
            img)  # img_offset: B×C×W×H , C=3(direct vector)+1(heatmap)+1(weight)

        img_code = torch.max(c4.view(c4.size(0), c4.size(1), -1), -1, keepdims=True)[0]  # 提取图像的关键特征
        B, C, H, W = pc_img_feat.size()
        img_down = F.interpolate(img, [H, W])
        B, _, N = pc1.size()
        ...
        # latents = self.fold1(code)  # 得到的潜在特征。(32,1280,14) --> (32,512,14)
        # x = F.relu(self.bn1(self.conv1(latents)))
        ...
        return latents,joints, pc1, feat1
    ...
    def forward(self, pc, feat, img, loader, center, M, cube, cam_para):
        ...
        embed,joint, pc1, feat1= self.encode(pc, feat, img, loader, center, M, cube, cam_para)
        # joint = torch.randn_like(joint) * 0.1 + joint  # (B, d, J) original training
        # joint = torch.randn_like(joint) * 3 + joint  # (B, d, J)
        ...
        for i in range(self.stacks):
            ...
            embed = self.trans[i](joint, pc1, embed, feat1)
            # embed = self.crossattn[i](joint, pc1, embed, feat1)
            joint = self.regress(embed)
        return joint
    ...
    def get_loss(self, pc, feat, img, loader, center, M, cube, cam_para, gt):
        ...
        embed,joint, pc1, feat1 = self.encode(pc, feat, img, loader, center, M, cube, cam_para)
        # weight = 0.5
        # loss = smooth_l1_loss(joint, gt) + weight * spatial_depth_ordering_loss(joint,gt,sketch,1.0)
        loss = smooth_l1_loss(joint, gt)
        ########################################################################################
        times = torch.zeros(
            (joint.size(0),), device=joint.device).float().uniform_(0.5, 1)
